-- Stop rather than discard any encrypted data on another installation.
do $$ begin
  if exists(select 1 from personnel_private.employees where resident_ciphertext is not null) then
    raise exception 'Encrypted resident numbers exist. Convert them securely before this migration.';
  end if;
end $$;
alter table personnel_private.employees rename column resident_ciphertext to resident_number;
alter table personnel_private.employees
  add column bank_name text not null default '' check(length(bank_name)<=100),
  add column bank_account text not null default '' check(length(bank_account)<=50 and bank_account ~ '^[0-9 -]*$'),
  add constraint personnel_resident_number_format check(resident_number is null or resident_number ~ '^[0-9]{6}[1-8][0-9]{6}$');

create or replace function public.personnel_query(p_workspace_id uuid,p_actor_id uuid,p_id uuid default null,p_page integer default 0,p_q text default '',p_status text default 'all')
...
create or replace function public.personnel_reveal(p_workspace_id uuid,p_actor_id uuid,p_id uuid) returns text
...
create or replace function public.personnel_save(p_workspace_id uuid,p_actor_id uuid,p jsonb) returns uuid
language plpgsql security definer set search_path=public,pg_temp as $$
