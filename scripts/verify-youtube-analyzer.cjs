...
(async()=>{
  ...
  try {
    ...
    let posted=false,deleted=false;
    server=http.createServer(async(req,res)=>{
      if(req.url==='/bundle.js'){res.setHeader('Content-Type','text/javascript');res.end(bundle.outputFiles[0].text);}
        ...
        else if(req.method==='DELETE'){deleted=true;res.end(JSON.stringify({deleted:'channel-b'}));}
        else if(req.url.includes('channelId'))res.end(JSON.stringify({videos:[video]}));
        else if(req.url.includes('batchId'))res.end(JSON.stringify({batch,runs:deleted?[existing]:[existing,added],hasMore:false,requests:[{id:'1',input_url:'https://youtube.com/@한글',status:'completed'},{id:'2',input_url:'bad',status:'failed',error_code:'INVALID_URL'}]}));
    ...
    assert.equal(await page.getByLabel('분석할 YouTube URL').count(),0);
    ...
    await page.getByRole('button',{name:'새 채널 삭제'}).click();
    await page.getByRole('alertdialog').getByRole('button',{name:'삭제'}).click();
    await page.getByText('새 채널',{exact:true}).waitFor({state:'detached'});
    assert.equal(deleted,true);
    assert.equal(await page.locator('tbody').first().locator('tr').count(),1);
    ...
    console.log('PASS: analyzed-only main page, add dialog, cumulative order, channel detail, deletion, batch progress, reload restoration, desktop/mobile containment');
