...
(async () => {
  ...
  try {
    ...
    for (const value of ['30%', '6.67%', '2%', '500%']) await page.getByText(value, { exact: true }).waitFor();
    const stages = page.getByLabel('웨비나 전환 단계');
    await stages.getByText('300명', { exact: true }).waitFor();
    assert.equal(await stages.getByText('200명', { exact: true }).count(), 0);
