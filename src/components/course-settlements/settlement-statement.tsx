...
import { printStatementWithEvidence } from "@/lib/course-settlements/print-evidence";
...
function Metric({ label, value, unit = "원", emphasized = false, formula, tone }: {
  label: string; value: number; unit?: "원" | "건"; emphasized?: boolean; formula?: string; tone?: "instructor" | "company";
}) {
  const color = tone === "instructor" ? "border-emerald-500/40 bg-emerald-500/10" : tone === "company" ? "border-blue-500/40 bg-blue-500/10" : emphasized ? "border-primary/40 bg-primary/5" : "";
  return <Card className={`min-w-0 ${color}`}><CardContent className={tone ? "p-3 sm:p-5" : "p-5"}>
    <p className="text-sm text-muted-foreground">{label}</p>
    {formula ? <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{formula}</p> : null}
    <p className={`mt-2 font-semibold tabular-nums ${tone ? "break-all text-lg sm:text-2xl" : "text-2xl"}`}>{roundWon(value).toLocaleString("ko-KR")}{unit}</p>
  </CardContent></Card>;
}
...
export function SettlementStatement({ instructor, monthlyAnalyses, courseId, courseName, courseCosts, appliedCourseCosts, costsAreSnapshot, draft: savedDraft, onChange, onSave, onConfirm, onReopen, busy }: {
  ...
  return <div className="space-y-5">
    ...
    <Card><CardHeader><CardTitle>최종 정산 계산</CardTitle><CardDescription>공동비용은 비용 탭에 저장된 회사·강사 부담 비율로 차감합니다.</CardDescription></CardHeader><CardContent className="space-y-5">
      <div className="grid gap-4 md:grid-cols-3">
        <Metric label="정산대상 매출" value={calculation.settlementTargetRevenue} formula="전체 매출 - PG 수수료 - 노바 수수료" />
        <Metric label="공동 부담 비용" value={calculation.costs.shared} formula="정산에 반영된 공동 부담 비용 합계" />
        <Metric label="분배 기준액" value={calculation.distributionBase} formula="정산대상 매출 - 공동 부담 비용" />
        <Metric label="강사 기본분" value={calculation.instructorBase} formula={`분배 기준액 × 강사 배분율 (${calculation.instructorRatioPercent}%)`} />
        <Metric label="강사 부담 비용" value={calculation.costs.instructor} formula="정산에 반영된 강사 단독 부담 비용 합계" />
        <Metric label="강사 공급가액" value={calculation.instructorSupply} formula="강사 기본분 - 강사 부담 비용 - 강사 공동비용 조정액" />
        <Metric label="VAT 10%" value={calculation.vat} formula="강사 공급가액 × 10%" />
        <Metric label="회사 기본분" value={calculation.companyBase} formula={`분배 기준액 × 회사 배분율 (${calculation.companyRatioPercent}%)`} />
        <Metric label="회사 부담 비용" value={calculation.costs.company} formula="정산에 반영된 회사 단독 부담 비용 합계" />
      </div>
      <p className="text-xs leading-relaxed text-muted-foreground">강사 공동비용 조정액 = 저장된 강사 부담액 - (공동 부담 비용 × 강사 배분율, 원 단위 반올림). 회사 공동비용 조정액 = 저장된 회사 부담액 - (공동 부담 비용 - 위에서 반올림한 강사분). 계산 단계별로 원 단위 반올림을 적용합니다.</p>
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        <Metric label="최종 강사 지급액" value={calculation.instructorFinal} formula="강사 공급가액 + VAT 10%" tone="instructor" />
        <Metric label="최종 회사 지급액" value={calculation.companyFinal} formula="회사 기본분 - 회사 부담 비용 - 회사 공동비용 조정액" tone="company" />
      </div>
      {missingEvidence.length?<Alert variant="destructive"><AlertTriangle/><AlertTitle>필수 증빙 미등록 {missingEvidence.length}건</AlertTitle><AlertDescription>{missingEvidence.map((cost)=>cost.name).join(", ")}</AlertDescription></Alert>:null}{!confirmed?<div><Label>예외 확정 사유</Label><Textarea placeholder="필수 증빙 없이 확정해야 하는 사유를 기록하세요." value={draft.exceptionReason} onChange={(event)=>patchDraft({exceptionReason:event.target.value})}/></div>:draft.exceptionReason?<Alert><AlertTitle>예외 확정 사유</AlertTitle><AlertDescription>{draft.exceptionReason}</AlertDescription></Alert>:null}<div className="flex justify-end">{confirmed?<Button variant="outline" disabled={busy} onClick={()=>void onReopen()}>{busy?<Loader2 className="animate-spin"/>:null}정산 확정 취소</Button>:<Button disabled={busy} onClick={()=>void run(()=>onConfirm(draft),"정산을 확정했습니다.")}>{busy?<Loader2 className="animate-spin"/>:<CheckCircle2/>}정산 확정</Button>}</div></CardContent></Card>
  </div>;
}
