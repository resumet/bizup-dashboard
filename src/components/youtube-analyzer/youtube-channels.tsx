...
export function YoutubeChannels({ maxUrls = 50 }: { maxUrls?: number }) {
  ...
  return <main className="mx-auto min-h-screen max-w-[1600px] space-y-6 px-4 py-6 sm:px-8">
    <header className="flex flex-wrap items-center justify-between gap-4 border-b pb-5">
      ...
      <Button variant="outline" size="icon" title="새로고침" aria-label="새로고침" onClick={()=>setRevision(value=>value+1)}><RefreshCw className="size-4"/></Button>
    </header>

    <section className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <label htmlFor="youtube-urls" className="font-semibold">분석할 YouTube URL</label>
        <span className={urls.length>maxUrls ? "text-sm text-red-700" : "text-sm text-muted-foreground"}>{urls.length} / {maxUrls}</span>
      </div>
      <textarea id="youtube-urls" value={text} onChange={event=>setText(event.target.value)} rows={4} maxLength={100000} placeholder={"https://www.youtube.com/@채널이름\nhttps://www.youtube.com/watch?v=xxxxxxxxxxx"} className="w-full resize-y rounded-md border bg-background p-3 text-sm leading-6 focus:outline-2 focus:outline-red-500" />
      {invalid.length>0 && <ul className="space-y-1 text-sm text-amber-800">
        {invalid.slice(0,5).map(({url,message})=><li key={url} className="break-all">{url}: {message}</li>)}
        {invalid.length>5 && <li>외 {invalid.length-5}개</li>}
      </ul>}
      <div className="flex justify-end"><Button onClick={analyze} disabled={submitting || active(batch) || !urls.length || urls.length>maxUrls}>{submitting ? <LoaderCircle className="size-4 animate-spin"/> : <Play className="size-4"/>}분석 시작</Button></div>
    </section>
    ...
    <section className="space-y-4 border-t pt-5">
      ...
      {batch && <div className="space-y-2 bg-muted/40 p-3 text-sm" aria-live="polite">
        <div className="flex flex-wrap gap-x-5 gap-y-2">
          <strong>{statuses[batch.status]}</strong><span>입력 {batch.input_count}개</span><span>고유 채널 {batch.unique_channel_count}개</span><span>처리 {completed} / {batch.input_count}</span><span>시작 {date(batch.created_at)}</span>{batch.completed_at && <span>완료 {date(batch.completed_at)}</span>}
        </div>
        <progress aria-label="분석 진행률" value={completed} max={batch.input_count} className="h-2 w-full accent-red-600"/>
      </div>}
      {requests.length>0 && <details open={active(batch)} className="text-sm">
        <summary className="cursor-pointer py-2">URL별 처리 상태 ({requests.length})</summary>
        <ul className="max-h-56 space-y-2 overflow-auto py-2">{requests.map(request=><li key={request.id} className="flex flex-wrap gap-2 border-b pb-2">
          <span className="min-w-0 flex-1 break-all">{displayUrl(request.input_url)}</span>
          <span className={request.status==="failed" ? "text-red-700" : "text-emerald-700"}>{statuses[request.status]}</span>
          {request.error_code && <p className="w-full text-red-700">{errorMessages[request.error_code] ?? "분석에 실패했습니다."}</p>}
        </li>)}</ul>
      </details>}
      {loading ? <p className="flex items-center gap-2 py-10 text-sm"><LoaderCircle className="size-4 animate-spin"/>분석 목록을 불러오는 중입니다.</p> : !runs.length ? <p className="border-y py-12 text-center text-muted-foreground">{active(batch) ? "첫 번째 채널 분석을 기다리고 있습니다." : "분석한 채널이 없습니다."}</p> : <div className="overflow-x-auto rounded-md border">
        <table className="w-full min-w-[1100px] text-sm">
          <thead className="bg-muted/60"><tr>{["채널 / 주소","전체 / 분석 영상","구독자","최고 조회 영상","최고 조회수","최고 1개 제외 평균","최근 20개 평균","등록 / 업데이트","상세"].map(heading=><th key={heading} className="whitespace-nowrap px-3 py-3 text-left font-medium">{heading}</th>)}</tr></thead>
          <tbody>{runs.map(run=><tr key={run.channel_id} className="border-t align-top hover:bg-muted/20">
            ...
            <td className="px-3 py-4"><Button variant="outline" size="sm" onClick={()=>openDetail(run)}>더보기</Button></td>
          </tr>)}</tbody>
        </table>
      </div>}
