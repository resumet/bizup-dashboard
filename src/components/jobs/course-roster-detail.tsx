...
export async function CourseRosterDetail({ jobId, embedded = false }: { jobId: string; embedded?: boolean }) {
  ...
  const { data: job } = await supabase
    .from("course_jobs")
    .select("id,name,default_course_name,status,latest_version,valid_count,error_count,updated_at,course_id,is_order_roster")
    .eq("id", jobId)
    .maybeSingle();
  if (!job) notFound();
