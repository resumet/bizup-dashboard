...
import { Loader2, Pencil, Save, StickyNote, Trash2 } from "lucide-react";
...
export function RosterNotesCard({
  jobId,
  currentUserId,
  currentUserEmail,
  initialNotes,
  ...
  const [deleteTarget, setDeleteTarget] = useState<CourseJobNote | null>(null);
  ...
  async function deleteNote() {
    ...
    try {
      const response = await fetch(
        `/api/jobs/${jobId}/notes/${deleteTarget.id}`,
        { method: "DELETE" },
      );
      const body = (await response.json()) as {
        id?: string;
    ...
    <Card>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1">
              명단 운영에 필요한 내용을 기록합니다. 작성자와 작성 시간이 함께 저장됩니다.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <form onSubmit={createNote} className="space-y-2">
          <Textarea
            aria-label="새 명단 메모"
            placeholder="명단에 공유할 메모를 입력하세요."
            maxLength={COURSE_JOB_NOTE_MAX_LENGTH}
            value={content}
          ...
          <div className="flex items-center justify-between gap-3">
            <span className="text-xs text-muted-foreground">
              {Array.from(content).length.toLocaleString("ko-KR")} / 2,000자
            </span>
            <Button
              type="submit"
              disabled={
                ...
                <article key={note.id} className="rounded-xl border p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="text-xs text-muted-foreground">
                      ...
                      {wasEdited ? (
                        <span>
                          {" "}· 수정 {formatNoteTime(note.updatedAt)}
                        </span>
                      ) : null}
                    </div>
                    {canManage && !isEditing ? (
