...
export function CourseWebinarEditor({ courseId }: { courseId: string }) {
  ...
  return <div className="space-y-6">
    ...
    <Card>
      <CardHeader><CardTitle>전환율과 광고 효율</CardTitle><CardDescription>현재 입력값 기준입니다. 라이브 전환율은 시작 인원, 구매 전환율은 결제 건수 기준이며 고유 구매자 비율과 다를 수 있습니다.</CardDescription></CardHeader>
      <CardContent>
        <div className="mb-6 grid gap-3 sm:grid-cols-3" aria-label="웨비나 전환 단계">
          {([['group_chat_count', '최종 단톡방', '명'], ['live_start_count', '라이브 시작', '명'], ['payment_count', '결제', '건']] as const).map(([key, label, unit], index) => <div className="rounded-xl border bg-muted/30 p-4" key={key}><p className="text-sm text-muted-foreground">{index + 1}. {label}</p><p className="mt-2 text-2xl font-semibold">{formatMetric(preview[key], unit)}</p></div>)}
        </div>
