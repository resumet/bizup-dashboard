...
export function PublicCourseStudentRoster({ courseName, students }: { courseName: string; students: OrderStudent[] }) {
  ...
  const chartItems = useMemo(() => [...summary.options].sort((a, b) => b.people - a.people || a.optionName.localeCompare(b.optionName, "ko-KR")), [summary.options]);
  const chartPeople = chartItems.reduce((sum, item) => sum + item.people, 0);
  let chartOffset = 0;
  const chartBackground = chartPeople
    ? `conic-gradient(${chartItems.map((item, index) => {
      const start = chartOffset;
      chartOffset += item.people / chartPeople * 100;
      return `${CHART_COLORS[index % CHART_COLORS.length]} ${start}% ${chartOffset}%`;
    }).join(", ")})`
    : "var(--muted)";
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-xl border bg-card p-5" aria-labelledby="option-chart-title">
        <div className="mb-5">
          <h2 id="option-chart-title" className="font-semibold">옵션별 수강생</h2>
          <p className="mt-1 text-sm text-muted-foreground">같은 옵션의 중복 결제는 한 명으로 계산합니다.</p>
        </div>
        <div className="grid items-center gap-6 md:grid-cols-[220px_1fr]">
          <div
            ...
            role="img"
            aria-label={`옵션별 수강생 분포: ${chartItems.map((item) => `${item.optionName || "옵션 없음"} ${item.people}명`).join(", ") || "데이터 없음"}`}
          >
            <div className="absolute inset-[27%] flex flex-col items-center justify-center rounded-full bg-card shadow-sm">
              <span className="text-2xl font-semibold tabular-nums">{chartPeople.toLocaleString("ko-KR")}</span>
              <span className="text-xs text-muted-foreground">옵션별 합계</span>
            </div>
          </div>
          <ul className="grid gap-3 sm:grid-cols-2">
            {chartItems.map((item, index) => (
              <li key={item.optionName} className="flex items-center gap-3 rounded-lg border p-3">
                <span className="size-3 shrink-0 rounded-full" style={{ backgroundColor: CHART_COLORS[index % CHART_COLORS.length] }} />
                <span className="min-w-0 flex-1 truncate text-sm">{item.optionName || "옵션 없음"}</span>
                <span className="whitespace-nowrap font-medium tabular-nums">
                  {item.people.toLocaleString("ko-KR")}명
                  <span className="ml-1 text-xs font-normal text-muted-foreground">({chartPeople ? (item.people / chartPeople * 100).toFixed(1) : "0.0"}%)</span>
                </span>
              </li>
            ))}
            {!chartItems.length ? <li className="text-sm text-muted-foreground">표시할 옵션이 없습니다.</li> : null}
          </ul>
        </div>
      </section>
