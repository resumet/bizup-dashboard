...
export function HrTaskBoard({
  ...
  const carriedCount = openTasks.filter((task) => task.planned_date < today).length;
  ...
  async function createTask() {
    ...
    try {
      const task = await readJson<WorkTask>(await fetch("/api/hr/tasks", {
        ...
        body: JSON.stringify({ title, description, assigneeId, plannedDate }),
      }));
      setTasks((current) => [task, ...current]);
      setTitle(""); setDescription(""); setAssigneeId(userId); setPlannedDate(today); setReview(null);
  ...
  return <div className="mx-auto max-w-5xl space-y-6 px-5 py-10">
    ...
    <Card><CardHeader><CardTitle>업무 추가</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
      ...
      <div className="grid gap-2"><Label htmlFor="task-date">업무일</Label><Input id="task-date" type="date" value={plannedDate} onChange={(event) => setPlannedDate(event.target.value)} /></div>
      <div className="grid gap-2"><Label>담당자</Label><Select value={assigneeId} onValueChange={setAssigneeId} disabled={!isAdmin}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{people.map((person) => <SelectItem key={person.id} value={person.id}>{person.name}</SelectItem>)}</SelectContent></Select></div>
