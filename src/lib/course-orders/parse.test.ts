...
test("가상계좌 입금대기는 합계에서 제외하고 분할결제의 완료 금액은 유지한다", () => {
  const paid = { 결제ID: "paid", 주문상태: "결제완료", 결제방법: "카드", 결제금액: 100, 환불금액: 0, "현 결제금액": 100 };
  const pending = { 결제ID: "pending", 주문상태: "입금대기", 결제방법: "가상계좌", 결제금액: 200, 환불금액: 0, "현 결제금액": 200 };
  const [merged] = parseCourseOrders(splitMatrix([paid, pending]));
  assert.equal(merged.paymentAmount, 100);
  assert.equal(merged.currentAmount, 100);
  const ordinary = parseCourseOrders(matrix(pending));
  assert.equal(ordinary[0].paymentAmount, 200);
  assert.equal(summarizeCourseOrders(ordinary).paymentAmount, 0);
  assert.equal(summarizeCourseOrders([{...ordinary[0], status:"결제완료"}]).paymentAmount, 200);
