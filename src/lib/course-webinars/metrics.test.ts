...
test("dashboard weights by audience totals and compares matched cohorts for partial records", () => {
  ...
  const result = summarizeWebinars([
    course({ group_chat_count: 100, live_start_count: 50, payment_count: 10, ad_spend: 100, revenue: 1000 }),
    course({ group_chat_count: 900, live_start_count: 90, payment_count: 9, ad_spend: 900, revenue: 900 }),
  ...
  assert.equal(result.ratios[1].value, 19 / 140 * 100);
