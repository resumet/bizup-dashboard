...
const order = (overrides: Partial<SavedCourseOrder> = {}): SavedCourseOrder => ({ id: "order", updatedAt: "", productName: "강의", optionName: "", memberName: "홍길동", phone: "010-1234-5678", email: "", paymentAmount: 100, refundAmount: 0, currentAmount: 100, status: "결제완료", paymentMethod: "카드", rs: "", adMedia: "", inflowType: "", paymentId: "", refundDate: "", orderId: "", ...overrides });
const toss = (buyer: string, amount: number): InstructorSourceDetails["toss"][number] => ({ buyer, amount, date: "2026-09-01", paymentMethod: "카드", status: amount < 0 ? "취소" : "승인", agency: "", pgFee: 0, supplyAmount: 0, vat: 0, acquiringStatus: "" });
...
test("양방향 누락과 금액 차이, 이름 정규화 및 원본 중복 보존", () => {
  const result = compareSettlementRoster([order(), order({ memberName: "주문전용" }), order({ memberName: "차이" })], [month({ toss: [toss("홍 길동", 70), toss("홍길동", 30), toss("정산전용", 100), toss("차이", 90)] })], "강사");
...
test("토스 취소와 무통장 음수 취소를 누적 엔진과 같은 부호로 합산", () => {
  ...
  assert.deepEqual(result.settlementTotal, { payment: 200, refund: 70, net: 130 });
});
...
test("동명이인 연락처 충돌과 이름 없는 거래를 자동 일치시키지 않는다", () => {
  const result = compareSettlementRoster([order(), order({ phone: "01099999999" }), order({ memberName: "" })], [month({ toss: [toss("홍길동", 200), toss("", 100)] })], "강사");
  assert.equal(result.find(row => row.name === "홍길동")?.status, "review");
  assert.deepEqual(result.filter(row => row.name === "이름 없음").map(row => row.status).sort(), ["orders-only", "settlement-only"]);
});
