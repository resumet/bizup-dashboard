...
test("토스 E열 주문번호를 이름과 별개로 보존한다", () => {
  const input = workbook();
  input.sheets[0].data[1][8] = "동명이인";
  input.sheets[0].data[1][4] = "001-ID";
  input.sheets[0].data[1][9] = "different-buyer-1";
  input.sheets[0].data[2][8] = "동명이인";
  input.sheets[0].data[2][4] = "002-ID";
  input.sheets[0].data[2][9] = "different-buyer-2";
  const rows = analyzeWorkbook(input).detailsByInstructor["김강사"].toss;
  assert.deepEqual(rows.slice(0, 2).map(row => row.orderNumber), ["001-ID", "002-ID"]);
});
