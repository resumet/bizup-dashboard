...
export default async function MessageHistoryDetailPage({ params }: PageProps) {
  ...
  const { data: batches, error: batchError } = detail.isTest
    ? { data: [], error: null }
    : await supabase
        .from("message_provider_batches")
        .select("id,chunk_index,recipient_count,success_count,failed_count,status,http_status,provider_status,delivery_checked_at")
  ...
  return <main className="min-h-screen">
    ...
    <div className="mx-auto max-w-[1600px] px-5 py-8 lg:px-8">
      ...
      {(batches ?? []).length > 0 ? <Card className="mt-5 overflow-hidden"><CardHeader><CardTitle className="text-base">DirecTalk 발송 배치</CardTitle></CardHeader><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>배치</TableHead><TableHead className="text-right">대상</TableHead><TableHead className="text-right">성공</TableHead><TableHead className="text-right">실패</TableHead><TableHead>상태</TableHead><TableHead>공급자 상태</TableHead><TableHead>접수 응답</TableHead><TableHead>결과 확인</TableHead></TableRow></TableHeader><TableBody>{(batches ?? []).map((batch) => <TableRow key={batch.id}><TableCell>#{batch.chunk_index + 1}</TableCell><TableCell className="text-right">{batch.recipient_count}</TableCell><TableCell className="text-right text-emerald-600">{batch.success_count}</TableCell><TableCell className="text-right text-destructive">{batch.failed_count}</TableCell><TableCell><StatusBadge status={batch.status} /></TableCell><TableCell className="font-mono text-xs">{batch.provider_status || "-"}</TableCell><TableCell className="font-mono text-xs">{batch.http_status ? `HTTP ${batch.http_status}` : "-"}</TableCell><TableCell className="whitespace-nowrap text-muted-foreground">{batch.delivery_checked_at ? formatDateTime(batch.delivery_checked_at) : "확인 중"}</TableCell></TableRow>)}</TableBody></Table></div></Card> : null}
      <Card className="mt-5 overflow-hidden"><CardHeader><CardTitle className="text-base">수신자별 실제 발송 결과</CardTitle></CardHeader><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>수신자</TableHead><TableHead>연락처</TableHead><TableHead>발송 시간</TableHead><TableHead>실제 결과</TableHead><TableHead>공급자 상태</TableHead><TableHead>최종 채널</TableHead><TableHead>응답 코드</TableHead><TableHead>실패 사유</TableHead></TableRow></TableHeader><TableBody>{detail.recipients.map((recipient) => <TableRow key={recipient.id}><TableCell className="font-medium">{recipient.name || "-"}</TableCell><TableCell className="font-mono">{formatPhone(recipient.phone)}</TableCell><TableCell className="whitespace-nowrap text-muted-foreground">{recipient.requestedAt ? formatDateTime(recipient.requestedAt) : formatDateTime(detail.createdAt)}</TableCell><TableCell><StatusBadge status={recipient.status} /></TableCell><TableCell className="font-mono text-xs">{recipient.providerStatus || "-"}</TableCell><TableCell>{finalMessageTypeLabel(recipient.finalMessageType)}</TableCell><TableCell className="font-mono text-xs">{[recipient.httpStatus && `HTTP ${recipient.httpStatus}`, recipient.providerResultCode || recipient.shoongCode].filter(Boolean).join(" / ") || "-"}</TableCell><TableCell className="max-w-md text-sm text-muted-foreground">{recipient.failureReason || recipient.providerResultMessage || "-"}</TableCell></TableRow>)}</TableBody></Table></div></Card>
    </div>
  </main>;
}

async function loadMessageDetail(supabase: Awaited<ReturnType<typeof createClient>>, jobId: string, messageId: string) {
  const { data: message } = await supabase.from("message_jobs").select("id,template_key,template_code,target_scope,requested_count,success_count,failed_count,status,provider,delivery_checked_at,created_at").eq("id", messageId).eq("course_job_id", jobId).maybeSingle();
  if (!message) return null;
  const { data: recipientRows } = await supabase.from("message_recipients").select("id,enrollment_id,normalized_phone,status,http_status,shoong_code,provider_status,provider_result_code,provider_result_message,final_message_type,delivery_checked_at,group_id,message_id,failure_reason,requested_at,completed_at").eq("message_job_id", message.id).order("requested_at");
  ...
  const recipients: RecipientDetail[] = (recipientRows ?? []).map((recipient) => ({ id: recipient.id, name: enrollmentValues.get(recipient.enrollment_id)?.customerName ?? "", phone: recipient.normalized_phone, requestedAt: recipient.requested_at, completedAt: recipient.completed_at, status: recipient.status, httpStatus: recipient.http_status, shoongCode: recipient.shoong_code, providerStatus: recipient.provider_status, providerResultCode: recipient.provider_result_code, providerResultMessage: recipient.provider_result_message, finalMessageType: recipient.final_message_type, deliveryCheckedAt: recipient.delivery_checked_at, failureReason: recipient.failure_reason, messageId: recipient.message_id, groupId: recipient.group_id }));
  return { isTest: false, templateKey: message.template_key as MessageHistoryItem["templateKey"], templateCode: message.template_code, provider: message.provider, deliveryCheckedAt: message.delivery_checked_at, targetScope: message.target_scope, requestedCount: message.requested_count, successCount: message.success_count, failedCount: message.failed_count, status: message.status, createdAt: message.created_at, recipients };
}
...
async function loadTestDetail(supabase: Awaited<ReturnType<typeof createClient>>, jobId: string, auditId: string) {
  ...
  const recipient: RecipientDetail = { id: `test-${audit.id}`, name: typeof metadata.recipient_name === "string" ? metadata.recipient_name : "권정인", phone: "01023787490", requestedAt: audit.created_at, completedAt: audit.created_at, status: success ? "success" : "failed", httpStatus: typeof metadata.http_status === "number" ? metadata.http_status : null, shoongCode: typeof metadata.shoong_code === "string" ? metadata.shoong_code : null, providerStatus: null, providerResultCode: null, providerResultMessage: null, finalMessageType: null, deliveryCheckedAt: null, failureReason: typeof metadata.failure_reason === "string" ? metadata.failure_reason : null, messageId: typeof metadata.message_id === "string" ? metadata.message_id : null, groupId: typeof metadata.group_id === "string" ? metadata.group_id : null };
  return { isTest: true, templateKey: template, templateCode: typeof metadata.template_code === "string" ? metadata.template_code : "", provider: typeof metadata.provider === "string" ? metadata.provider : "-", deliveryCheckedAt: null, targetScope: "test", requestedCount: 1, successCount: success ? 1 : 0, failedCount: success ? 0 : 1, status: success ? "completed" : "failed", createdAt: audit.created_at, recipients: [recipient] };
}

function StatusBadge({ status }: { status: string }) { if (status === "success" || status === "completed") return <Badge><CheckCircle2 />성공</Badge>; if (["unknown", "pending", "submitted", "processing"].includes(status)) return <Badge variant="secondary"><Clock3 />{status === "pending" || status === "submitted" ? "대기" : "결과 확인 중"}</Badge>; if (status === "partial_failed") return <Badge variant="destructive"><XCircle />일부 실패</Badge>; return <Badge variant="destructive"><XCircle />실패</Badge>; }
function finalMessageTypeLabel(value: string | null) { if (value === "AT") return "알림톡"; if (value === "SM") return "SMS"; if (value === "LM") return "LMS"; return value || "-"; }
