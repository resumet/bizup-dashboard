...
`supabase/migrations/202609110001_roster_message_sources.sql`을 적용하면 알림톡·문자 자동화에서 주소록 아래 구분선으로 나뉜 **수강생 명단**을 발송 대상으로 선택할 수 있습니다. 분석 완료된 명단의 최신 버전을 읽으며, 전화번호가 중복되면 첫 번째 수강생에게 한 번만 발송합니다. 미리보기는 같은 기준으로 첫 10명을 표시합니다. 발송 요청 시 수신자를 확정하여 이후 명단 수정이 진행 중인 발송에 영향을 주지 않으며, 발송 이력에서 원본 명단과 연결된 결과를 확인할 수 있습니다.
